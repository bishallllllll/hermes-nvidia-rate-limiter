...
def call_llm(
    ...
    *,
    ...
    _p = resolved_provider
    cached = get_cached_response(_p, kwargs)
    if cached is not None:
        return cached

    try:
        with ProviderSlot(_p):
            _r = _validate_llm_response(client.chat.completions.create(**kwargs), task)
            if _r:
                set_cached_response(_p, kwargs, _r)
            return _r
