...
from agent.global_rate_limiter import ProviderSlot, get_cached_response, set_cached_response
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    def _call():
        ...
        cached = get_cached_response(_provider, api_kwargs)
        if cached is not None:
            result["response"] = cached
            return

        try:
            with ProviderSlot(_provider):
                ...
                if result["response"]:
                    set_cached_response(_provider, api_kwargs, result["response"])
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions():
        ...
        # Add model name to the normalized response so title generator
        # and other downstream hooks see the actual model used.
        if model_name:
            setattr(result["response"], "model", model_name)
